    ...
    public class Player : Entity
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (ObjectsToDestroy.Length > 0 || CollidingLines.Length > 0)
            {
                ...
                DataBatchToSearch = new();
            }
            PreviousVelocity = new Vector2(0, 0);
            foreach (Sprite sprite in ObjectsToDestroy)
            {
                GameRunning.PlayerNeuralNetwork.AccumulatedRewards -= 300;
                for (int i = 0; i < DataBatchToSearch.Count; i++)
                {
                    GameRunning.PlayerNeuralNetwork.CreateExpectedOutputs(DataBatchToSearch[i], (float)(1f / Math.Sqrt(i)));
                }
                GameRunning.PlayerNeuralNetwork.Learn([.. DataBatchToSearch]);
